...
function normalizeGatewayPath(rawPath) {
  ...
  const decoded = String(rawPath).trim();
  if (!decoded) return null;
  if (decoded.startsWith("/")) return decoded;
  return `/${decoded}`;
}

function extractGatewayPath(req) {
  try {
    const requestUrl = new URL(req.url || "/", "http://localhost");
    return normalizeGatewayPath(requestUrl.searchParams.get("path"));
  } catch {
    return null;
  }
}
...
export default async function handler(req, res) {
  ...
  const gatewayPath = extractGatewayPath(req);
  if (gatewayPath) {
    req.url = gatewayPath;
  }
